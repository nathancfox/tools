...
display_palette <- function(palette) {
    ...
    plt <- (
        ...
        + ggplot2::scale_y_continuous(limits = c(0, 0.5), expand = ggplot2::expansion(mult = c(0, 0)))
        + ggplot2::coord_flip()
...
#' Augment cat to print to both stdout and a file.
#'
#' Wrapper around \code{base::cat()} to allow printing
#' to a file as well as stdout. If file is "", then
#' this function behaves exactly the same as a normal
#' \code{cat()} printing to stdout. If file is not "",
#' then the \code{...} arguments will be printed to the
#' stdout AND to the file.
#'
#' @param ... Passed directly to \code{cat()}.
#' @param file If "", then this function behaves exactly like
#'   \code{cat()}. If not "", then \code{...} is printed to
#'   the file argument as well as to the stdout.
#' @param sep Passed directly to \code{cat()}.
#' @param fill Passed directly to \code{cat()}.
#' @param labels Passed directly to \code{cat()}.
#' @param append Passed directly to \code{cat()}.
#' @export
