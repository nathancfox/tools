...
calc_cluster_de_1v1 <- function(sce_obj, clust_ids, fc_thresh=0.5, p_thresh=1E-4,
    ...
    clust.mean <- t(apply(2^logcounts(sce_obj)-1, 1,
                          function(z) {sapply(split(z, clust_ids), mean)}))
    ...
    if (method == "mw") {
        ...
        for (i in 1:dim(combos)[2]) {
            ...
            pvals[[i]] <- apply(logcounts(sce_obj), 1,
        ...
        for (i in 1:dim(combos)[2]) {
            f1 <- (clust_ids == combos[1, i])
            f2 <- (clust_ids == combos[2, i])
            temp <- genefilter::rowttests(as.matrix(logcounts(sce_obj)[, (f1 | f2)]), as.factor(f1[f1 | f2]))
            pvals[[i]] <- temp[, 3]
        }
    }
...
#         logFC[, i] <- log2(clust.mean[, i] + 1) - log2(rowMeans(clust.mean[, -i]) + 1)
        logFC[, i] <- log2(clust.mean[, i] + 1) - log2(base::rowMeans(clust.mean[, -i], na.rm = TRUE) + 1)
    }
...
#' @export
write_de <- function(pct_list, roc_list, de_res,
...
read_de_results <- function(folder, batch_prefix = "batch_",
    ...
    combos <- data.frame(batches = as.numeric(batches),
