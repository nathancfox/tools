...
plot_de_results <- function(de_res, res_source = c("list", "table"), plot = c("volcano", "ma"),
                            ct_markers = NULL, correct = FALSE, plot_title = "DE Results",
                            alpha = 0.05, mean_expression = NULL) {
    valid_args <- list(res_source = c("list", "table"),
                       plot = c("volcano", "ma"))
    if (length(res_source > 1)) {
        res_source <- res_source[1]
    }
    if (length(plot > 1)) {
        plot <- plot[1]
    }
    if (!(res_source %in% valid_args[["res_source"]])) {
        stop(sprintf("res_source must be in [%s]\n", paste(valid_args[["res_source"]], collapse = ", ")))
    }
    if (!(plot %in% valid_args[["plot"]])) {
        stop(sprintf("plot must be in [%s]\n", paste(valid_args[["plot"]], collapse = ", ")))
    }
    if (res_source == "list") {
        gene_f <- !((is.na(de_res$pval[, 2])) | (is.na(de_res$logfc[, 2])))
        data <- data.frame(pval = de_res$pval[, 2],
                           logfc = de_res$logfc[, 2],
                           mean_exp = de_res$clust_exp[, 2])
        num_genes <- dim(data)[1]
    } else if (res_source == "table") {
    ...
    if (correct) {
        ...
        data$pval <- data$pval * num_genes
    }
    if (plot == "volcano") {
        data$pval <- log10(data$pval)
        if (correct) {
            y_label_correct <- " - Bonferroni Correction"
        } else {
            y_label_correct <- ""
        }
        p <- (
            ...
            + ylab(paste0("log10 p-value ", y_label_correct))
            + theme(legend.title = element_blank())
         )
        if (!(is.null(ct_markers))) {
            p <- (
                p
                + geom_point(aes(x = logfc, y = pval, color = known, size = known))
                + scale_colour_manual(name = "Marker",
                                      label = c("Known", "Novel"),
                                      values = c("#E00202", "#000000"))  # black and red
                + scale_size_manual(values = c(4, 0.5), guide = "none")
            )
        } else {
            p <- (
                p
                + geom_point(aes(x = logfc, y = pval), size = known)
            )
        }
    } else if (plot == "ma") {
        ...
        if (correct) {
            pvalue_label <- " (BF Corrected)"
        } else {
            pvalue_label <- ""
        }
        p <- (
            ggplot(data = data)
            + scale_colour_manual(name = paste0("p-value", pvalue_label),
                                  values = c("#E00202", "#000000"))  # black and red
            + scale_size_manual(values = c(4, 0.5), guide = "none")
        ...
        if (!(is.null(ct_markers))) {
            p <- (
                p
                + geom_point(aes(x = mean_exp, y = logfc, color = significant,
            ...
            p <- (
                p
                + geom_point(aes(x = mean_exp, y = logfc,
                             color = significant, size = significant))
            )
        } 
    } else {
        stop("invalid plot argument got by error check!")
    }
    return(p)
}
